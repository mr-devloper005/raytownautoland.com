...
export default async function HomePage() {
  // primaryTask = first enabled task that is NOT hidden from the public UI (→ sbm)
  ...
  const primaryPosts = uniquePosts(
    taskFeed.find(({ task }) => task.key === primaryTask)?.posts ||
      taskFeed.filter(({ task }) => !isUiHiddenTask(task.key)).flatMap(({ posts }) => posts),
  ).slice(0, 24)
  ...
  return (
    <EditableSiteShell>
      <main>
        ...
        <div className="mx-auto w-full max-w-[var(--editable-container)] px-6 py-10 sm:px-8 lg:px-10">
